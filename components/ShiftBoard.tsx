...
import { ChevronLeft, ChevronRight, Loader2, Filter, AlertTriangle, X, Calendar as CalendarIcon, Copy, Sun, Moon, Clock } from 'lucide-react';

const SHIFT_PERIODS: ShiftPeriod[] = ['07-13', '13-19', '19-00', '00-07'];

// Modal Component for Shift Editing
...
const ShiftModal: React.FC<ShiftModalProps> = ({ isOpen, onClose, staff, date, currentShifts, onSave, allDays }) => {
  ...
  useEffect(() => {
    // Initialize with current state of the clicked day
  ...
  const handleSave = () => {
    // If not replicating, just pass the current day's weekday implicitly by passing empty replicate array logic 
    // actually simpler: if replicateMode is false, pass only current day index, or handle in parent.
    // Let's pass the weekdays to replicate. If empty and replicate is false, parent handles simple save.
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="bg-white dark:bg-gray-900 rounded-xl shadow-2xl border border-gray-200 dark:border-gray-700 w-full max-w-md overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="p-6 space-y-6 overflow-y-auto">
           ...
           {/* Manual Selection */}
           <div>
             <label className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-2 block">Seleção Manual de Horários</label>
           ...
           <div className="border-t border-gray-100 dark:border-gray-800 pt-4">
              ...
              >
                <Copy size={14} /> 
                {replicateMode ? "Cancelar Padrão de Repetição" : "Replicar Padrão..."}
              </button>
              ...
              {replicateMode && (
                <div className="bg-blue-50 dark:bg-blue-900/10 p-3 rounded-lg border border-blue-100 dark:border-blue-800/30 animate-in slide-in-from-top-2">
                   <p className="text-xs text-blue-800 dark:text-blue-300 mb-2 font-medium">Aplicar este horário para todos os:</p>
...
export const ShiftBoard: React.FC = () => {
  ...
  const handleShiftSave = async (periods: ShiftPeriod[], isExtra: boolean, replicateWeekdays: number[]) => {
    ...
    setLoading(true); // Show global loading or localized optimistic update
    
    // Determine dates to apply
    ...
    try {
        ...
        // Strategy: For each target date, we want to SET the shifts to exactly `periods`.
        // This means deleting existing shifts for that day that are NOT in `periods`, 
        // and upserting the ones that ARE in `periods`.
        // However, a simpler atomic approach per day: Delete all for that staff/day, Insert new ones.
        
        const operations = targetDates.map(async (date) => {
            const dateStr = formatDateISO(date);
            
            // 1. Delete all existing for this staff/date
  ...
  const getStaffStats = (staffMember: Staff) => {
    ...
    const weeklyStats = weeks.map((weekDays, index) => {
        ...
        // Filter shifts within this week range
  ...
  const getDailyStats = (date: Date) => {
    ...
    const staffIds = new Set(shiftsOnDay.map(s => s.staff_id));
    
    let doctors = 0, nurses = 0, techs = 0, drivers = 0;

    staffIds.forEach(id => {
      const person = staff.find(s => s.id === id);
      const roleName = person?.roles?.name.toLowerCase() || '';
      
      if (roleName.includes('médic')) doctors++;
      else if (roleName.includes('enferm') && !roleName.includes('téc')) nurses++;
      else if (roleName.includes('téc')) techs++;
      else if (roleName.includes('condutor') || roleName.includes('motorista')) drivers++;
    });
    return { doctors, nurses, techs, drivers };
  };
          ...
          <thead className="bg-white dark:bg-gray-900 sticky top-0 z-20 shadow-md">
            <tr>
              ...
              {days.map(day => {
                ...
                const total = stats.doctors + stats.nurses + stats.techs + stats.drivers;
                ...
                return (
                    <th key={day.toISOString()} className={`min-w-[50px] border-b border-r border-gray-200 dark:border-gray-800 py-2 px-1 text-center ${isWeekend ? 'bg-gray-50 dark:bg-gray-900/50' : 'bg-white dark:bg-gray-900'} align-bottom`}>
                      ...
                      <div className="flex flex-col items-center gap-0.5 mb-2">
                        <div className="flex flex-col items-center text-[8px] font-mono leading-tight text-gray-400 dark:text-gray-500">
                           {stats.doctors > 0 && <span className="text-cyan-600 dark:text-cyan-400 font-bold">{stats.doctors}</span>}
                           {stats.nurses > 0 && <span className="text-pink-600 dark:text-pink-400 font-bold">{stats.nurses}</span>}
                           {stats.techs > 0 && <span className="text-purple-600 dark:text-purple-400 font-bold">{stats.techs}</span>}
                           {stats.drivers > 0 && <span className="text-amber-600 dark:text-yellow-400 font-bold">{stats.drivers}</span>}
                           {total === 0 && <span className="text-gray-300 dark:text-gray-700">-</span>}
                        </div>
                      </div>
            ...
            {loading ? (
              ...
              return (
                <tr key={person.id} className={`${rowBg} hover:bg-blue-50/50 dark:hover:bg-blue-900/10 transition-colors group border-b border-gray-100 dark:border-gray-800`}>
                  ...
                  {/* Day Cells - Visual Blocks */}
                  {days.map(day => {
                     const dateStr = formatDateISO(day);
                     const isWeekend = day.getDay() === 0 || day.getDay() === 6;
                     const cellShifts = schedules.filter(s => s.staff_id === person.id && s.date === dateStr);
                     
                     // Analyze shifts for visuals
                     const hasDayShifts = cellShifts.some(s => s.shift_slot === '07-13' || s.shift_slot === '13-19');
                     const hasNightShifts = cellShifts.some(s => s.shift_slot === '19-00' || s.shift_slot === '00-07');
                      ...
                      >
                         <div className="w-full h-full flex flex-col gap-0.5">
                            ...
                            <div className={`flex-1 rounded-sm transition-all ${
                                hasDayShifts 
                                ? (isExtra ? 'bg-amber-400/90 border-l-2 border-green-400' : 'bg-amber-200 dark:bg-amber-600/60') 
                                : 'bg-transparent'
                            }`}></div>
                            ...
                            <div className={`flex-1 rounded-sm transition-all ${
                                hasNightShifts 
                                ? (isExtra ? 'bg-indigo-400/90 border-l-2 border-green-400' : 'bg-indigo-200 dark:bg-indigo-600/60') 
                                : 'bg-transparent'
                            }`}></div>
                         </div>
                      </td>
